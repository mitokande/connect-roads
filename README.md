...
Lay a single unbroken road from one border arrow to the other. The numbers
...
- Squares you've ruled out, and squares the clues have already settled, turn the
  road away for nothing.
- Drag back along the road to rub it out. Finish the route and five cars drive it.
...
| `npm run sfx:build` | Re-synthesise the sound effects into `assets/sfx/` |
...
small oscillator-and-envelope synth that bakes seventeen WAVs, so the whole set
weighs less than half a megabyte and is the same on every machine.
